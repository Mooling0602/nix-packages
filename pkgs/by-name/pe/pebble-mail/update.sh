...
if [ "$force" = false ] && [ "$current_version" = "$version" ]; then
  update_readme_versions
  echo "pebble-mail is already at $version"
  exit 0
fi

src_url="https://github.com/QingJ01/Pebble/releases/download/v${version}/Pebble_${version}_amd64.deb"
...
echo "Updated pebble-mail to $version"
