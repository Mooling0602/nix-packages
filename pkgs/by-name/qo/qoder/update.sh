...
# the electron-builder path `qoder-app/releases/<version>/`. Discovery uses the
# version-less `latest` alias; the version has a Debian epoch (`1:0.2.5`) that
# the download path and `package.nix` do not use, so it is stripped.
set -euo pipefail
...
#
# The `control` Version is `1:0.2.5` (Debian epoch) while the release path and
# `package.nix` use the bare `0.2.5`, so everything up to the last colon is
# stripped.
latest_version() {
  ...
  # Strip the Debian epoch (`1:0.2.5` -> `0.2.5`).
